...
export const getGrade = (score) => {
  if (score === null || score === undefined || score === '') return { grade: '-', remark: '-', remarkFr: '-', color: 'text-gray-400' };
  const s = parseFloat(score);
  if (s >= 18) return { grade: 'A+', remark: 'Excellent', remarkFr: 'Excellent', color: 'text-emerald-400' };
  if (s >= 16) return { grade: 'A', remark: 'Very Good', remarkFr: 'Très Bien', color: 'text-emerald-400' };
  if (s >= 14) return { grade: 'B+', remark: 'Good', remarkFr: 'Bien', color: 'text-blue-400' };
  if (s >= 12) return { grade: 'B', remark: 'Fairly Good', remarkFr: 'Assez Bien', color: 'text-blue-400' };
  if (s >= 10) return { grade: 'C', remark: 'Pass', remarkFr: 'Passable', color: 'text-yellow-400' };
  if (s >= 8) return { grade: 'D', remark: 'Poor', remarkFr: 'Insuffisant', color: 'text-orange-400' };
  return { grade: 'F', remark: 'Fail', remarkFr: 'Echec', color: 'text-red-400' };
};
...
export const getOverallRemark = (average) => {
  if (average === null || average === undefined) return { en: 'Needs Improvement', fr: 'Peut Mieux Faire' };
  if (average >= 16) return { en: 'Excellent', fr: 'Excellent' };
  if (average >= 14) return { en: 'Very Good', fr: 'Très Bien' };
  if (average >= 12) return { en: 'Good', fr: 'Bien' };
  if (average >= 10) return { en: 'Pass', fr: 'Passable' };
  if (average >= 8) return { en: 'Poor', fr: 'Insuffisant' };
