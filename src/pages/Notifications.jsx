...
export default function Notifications() {
  ...
  <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginBottom:"12px"}}>
    {[{id:"result",label:"📋 Result Ready"},{id:"fees",label:"💰 Fee Reminder"},{id:"meeting",label:"👥 PTA Meeting"},{id:"general",label:"📢 General"}].map(t=>(
      <button key={t.id} onClick={()=>setMsgType(t.id)} style={{padding:"6px 12px",background:msgType===t.id?"rgba(37,211,102,0.2)":"rgba(255,255,255,0.05)",border:"1px solid "+(msgType===t.id?"rgba(37,211,102,0.5)":"rgba(255,255,255,0.1)"),borderRadius:"8px",color:msgType===t.id?"#25d366":"#94a3b8",fontSize:"12px",cursor:"pointer"}}>{t.label}</button>
    ))}
  </div>
  ...
  {filtered.map(student => (<div key={student.id} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:"10px",padding:"14px",marginBottom:"8px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"8px"}}><div><div style={{fontSize:"14px",color:"#fff",fontWeight:"bold"}}>{student.name}</div><div style={{fontSize:"11px",color:"#64748b"}}>{student.level} · 📱 {student.parentPhone||"No parent phone"}</div></div><button onClick={()=>sendWhatsApp(student.parentPhone||student.phone||"",getMessage(student))} style={{padding:"8px 16px",background:"linear-gradient(135deg,#25d366,#128c7e)",border:"none",borderRadius:"8px",color:"#fff",fontSize:"12px",cursor:"pointer",fontWeight:"bold"}}>Send WhatsApp</button></div><div style={{marginTop:"8px",fontSize:"11px",color:"#475569"}}>{getMessage(student)}</div></div>))}
  </div></div>);
}
